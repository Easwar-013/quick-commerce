      ...
      <div className="mt-3 flex items-center justify-between pt-2 border-t border-gray-50">
        ...
        {isOutOfStock ? (
          ...
          <div
            ...
            className="flex items-center bg-emerald-600 text-white rounded-xl px-2 py-1 gap-2 shadow-xs"
          >
            <button
              onClick={() => updateQuantity(product._id, -1)}
              className="p-0.5 hover:opacity-75 focus:outline-none cursor-pointer"
            >
              <Minus className="w-3.5 h-3.5" />
            </button>
            <span className="text-xs font-bold min-w-[14px] text-center">{quantity}</span>
            <button
              onClick={() => updateQuantity(product._id, 1)}
              disabled={quantity >= product.stock}
              className="p-0.5 hover:opacity-75 focus:outline-none disabled:opacity-40 cursor-pointer"
            >
